...
namespace CUserPermissionDemo
{
    ...
    public class Program
    {
        ...
        private static readonly string endpointUrl = ConfigurationManager.AppSettings["CosmosDbEndpoint"];
        private static readonly string authorizationKey = ConfigurationManager.AppSettings["CosmosDbMasterKey"];
        ...
        private static async Task RunDemoAsync(string databaseId, string collectionId)
        {
            ...
            // Create two users
            User user1 = await client.CreateUserAsync(db.UsersLink, new User { Id = "Ram" });
            User user2 = await client.CreateUserAsync(db.UsersLink, new User { Id = "Tom" });
            ...
            ViewPermission(permissionUser1Col1);
            ...
            ViewPermission(permissionUser2Col2);
        ...
        private static async Task<Permission> CreatePermissionAsync(string resourceLink, string userLink, PermissionMode mode, 
            string resourcePartitionKey = null)
        {
        ...
        private static async Task AttemptReadFromTwoCollections(List<string> collectionLinks, List<Permission> permissions)
        {
            ...
            using (DocumentClient client = new DocumentClient(new Uri(endpointUrl), permissions))
            {
                ...
                catch (DocumentClientException de)
                {
                    ...
                    Console.WriteLine("User Doens not have only read permissions");
                }
            }
        ...
        private static async Task AttemptWriteWithReadPermissionAsync(string collectionLink, Permission permission)
        {
            using (DocumentClient client = new DocumentClient(new Uri(endpointUrl), permission.Token))
            {
                ...
                catch (DocumentClientException de)
                {
                    //expecting an Forbidden exception, anything else, rethrow
                    if (de.StatusCode != HttpStatusCode.Forbidden) throw;
                    Console.WriteLine("User Doens not have permissions to writing antyting on Collection {0}", collectionLink);
                }
            }
        }
        ...
        private static async Task AttemptAdminOperationsAsync(string collectionLink, Permission permission)
        {
            using (DocumentClient client = new DocumentClient(new Uri(endpointUrl), permission.Token))
            {
                ...
                catch (DocumentClientException de)
                {
                    ...
                    if (de.StatusCode != HttpStatusCode.Forbidden) throw;
                    Console.WriteLine("User Doens not have permissions to read antyting other than Collection {0}", collectionLink);
                }
            }
        }
